...
from tkinter import filedialog #b/c we will work on our file explorer a lot
...
class ImageToPDFConverter:
    def __init__(self, root):
        self.root = root # root is initialized
        self.image_paths = [] # variable for imgs
        self.output_pdf_name = tk.StringVar() 
        self.selected_images_listbox = tk.Listbox(root, selectmode=tk.MULTIPLE) # tk.MULTIPLE, b/c we are going to store multiple img names in the variable

        self.initialize_ui()

    def initialize_ui(self):
        title_label = tk.Label(self.root, text="Image to PDF Converter", font=("Helvetica", 16, "bold"))
        title_label.pack(pady=10) # padding for title_label
        ...
        pdf_name_entry = tk.Entry(self.root, textvariable=self.output_pdf_name, width=40, justify='center') #input section so that user can type the PDF name
        pdf_name_entry.pack()
    ...
    def update_selected_images_listbox(self):
        ...
        for image_path in self.image_paths: # for storing new names of the images
...
def main():
    root = tk.Tk()
    root.title("Image to PDF") # title for the img to pdf box screen
    converter = ImageToPDFConverter(root)
    root.geometry("400x600") # size of the screen
